...
## Lab 1 Overview
### First Half
Using the Rstudio (JupyterHub) server:
(TBA)
...
#### Comments and Docstrings (w.r.t JupyterNotebook Cells) (TBA)
...
```
Now in this case, we will pipe the msleep data frame to the function that will select two columns and then pipe the new data frame to the function head() which will return the head of the new data frame.
...
The geocode_OSM() geocodes a location (based on a search query) to coordinates and a bounding box. Similar to geocode from the ggmap package. It uses OpenStreetMap Nominatim. 
...
map_big <- get_stamenmap(
  rbind(xranges[1]+1,yranges[1]+3,xranges[2]-1,yranges[2]-1), 
  zoom = 8)
ggmap(map_big)
```
...
Now, let's try plottibf somethings over the map!
...
```{r}
library(feather)
filepath <- "/Volumes/you.y/MAST30034_R/data/"
```
...
destfile ="/Volumes/you.y/MAST30034_R/data/download.csv"
